...
const HttpTableRow = (props) => {
  ...
  const getRssFilters = props.getRssFilters;

  const checkSslWarn = props.checkSslWarn;
  const cookieExist = props.cookieExist;
  const getAssignGroupId = props.getAssignGroupId;
  ...
  return (
    <tbody>
      <tr className="py-4 mobile:grid mobile:gap-4 mobile:grid-cols-2 border-b last:border-b-0 border-gray-200 dark:border-gray-700 ">
        ...
        <td className={'pr-4 items-center text-base font-semibold text-gray-900 dark:text-white'}>
          {humanMonitorType()}
        </td>
        <td className="pr-4 flex-1 items-center text-base font-semibold text-gray-900 dark:text-white">
        ...
        <td className="row-start-3 col-start-3 ">
          ...
          {!monitor?.enabledPlugins?.ALL_PLUGINS.value &&
            ...
            <div className="dark:text-white text-black ">
              <Tooltip
                content={`Disabled plugin${Object.entries(monitor?.enabledPlugins)
                  .filter((e) => {
                    return e[1].value === false && e[0] !== "ALL_PLUGINS";
                  })
                  .at(1)
          ...
          {cookieExist ? (
            <OutsideHider state={dotsIsVisible} setstate={setDotsIsvisible}>
              ...
              <div
                ...
                className={`z-10 ${dotsIsVisible ? "absolute translate-x-[-75%]" : "hidden "
              ...
              >
                ...
                {monitor.monitor_type === "rss_parser" ? (
                  <button
                    ...
                    id={monitor.id}
                    onClick={(e) => {
                      getRssFilters(e.target.id);
                      setDotsIsvisible(false);
                    }}
                    data-modal-toggle="modal_delete-{{@index}}"
                    className="mx-3 my-2 flex text-left text-gray-900 dark:text-white hover:underline"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-6 w-6 pr-2"
